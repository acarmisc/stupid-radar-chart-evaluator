...
- author: single human contributor authorship. High = consistent personal style, organic naming, micro-edits, typo→fix patterns, idiomatic shortcuts, native-language comments.
- ai: LLM-generated code. High = boilerplate verbosity, defensive try/except everywhere, perfect docstrings on every function, banner-comment section headers, suspiciously uniform style, polished commit messages, em-dashes in comments, near-zero TODO/FIXME density, over-documentation, AI-named authors (Agentic/Bot/Copilot/Claude/etc), Co-Authored-By AI footer in commits.
...
1. VERBOSITY BIAS: well-structured, well-commented, idiomatic human code looks AI-like. Don't be fooled — humans CAN write clean code. Weight metadata (author names, commit patterns) over surface polish.
2. METADATA WEIGHT: explicit AI author identities (e.g. "Riccardo Agentic", "Copilot", "Cursor Agent", "Claude") and "Co-Authored-By: Claude" footers are DIRECT EVIDENCE — weight these heavily on the ai axis. A repo with 40% Co-Authored-By:Claude commits should score ai≥70 regardless of code polish.
